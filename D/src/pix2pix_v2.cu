...
namespace Const
{
__constant__ uint_t num_images;
__constant__ float alpha;
__constant__ float epsilon;
}

template <typename T>
__global__ void cuFill(uint_t n, const T v, T* buf)
{
...
__global__ void cuPreprocWeights_v2(uint_t n,
	const float *W,
	Tensor_t<float4> *Wv,
...
__global__ void cuPreprocInput_v2(uint_t n, 
...
__global__ void cuConv2D_v2(const Tensor_t<float4> *input, 
	...
	const Phase_t *Phase)
{
	uint_t H = input->nz, W = input->ny, Cv = input->nx, C = Cv * 4;
	uint_t R = Phase->GetKernelShape(0), S = Phase->GetKernelShape(1);
	uint_t Kv = output->nx, K = Kv * 4;

	uint_t OH = H / STRIDE, OW = W / STRIDE;

	uint_t kv = threadIdx.x + blockIdx.x * blockDim.x; uint_t k = kv * 4;
	uint_t ow = threadIdx.y + blockIdx.y * blockDim.y;
	uint_t oh = (threadIdx.z + blockIdx.z * blockDim.z) % OH;
	uint_t img = (threadIdx.z + blockIdx.z * blockDim.z) / OH;

	if (kv >= Kv || ow >= OW || oh >= OH || img >= Const::num_images) return;
	
	uint_t lidx = oh * OW * Kv + ow * Kv + kv;
	uint_t gidx = img * OH * OW * Kv + lidx;
	uint_t inp_offset = img * (H * W * Cv);

	float4 x = Phase->GetBias_v2(kv);
	
	uint_t rbeg = max((int) 0, PAD - (int) oh * STRIDE);
	uint_t rend = min((int) R, PAD + (int) H - (int) oh * STRIDE);
	uint_t sbeg = max((int) 0, PAD - (int) ow * STRIDE);
	uint_t send = min((int) S, PAD + (int) W - (int) ow * STRIDE);

	for (uint_t r = rbeg; r < rend; ++r) {
		uint_t ih = oh * STRIDE - PAD + r;
		for (uint_t s = sbeg; s < send; ++s) {
			uint_t iw = ow * STRIDE - PAD + s;
			for (uint_t cv = 0; cv < Cv; ++cv) {
				uint_t c = cv * 4;
...
__global__ void cuLeakyRelu_v2(uint_t n, 
	const Tensor_t<float4> *input,
	Tensor_t<float4> *output)
{

	uint_t idx = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void cuBatchNorm_v2(uint_t n,
	...
	const Phase_t *Phase)
{
	...
	uint_t H = inout->nz, W = inout->ny, Cv = inout->nx, C = Cv * 4;

	uint_t size = H * W * Cv;
	uint_t img = blockIdx.x / Cv;
	uint_t cv = blockIdx.x % Cv;

	uint_t img_offset = H * W * Cv * img;

	for (uint_t i = 0; i < H * W; i += blockDim.x) {
		uint_t idx = (i + threadIdx.x) * Cv + cv;
		if (idx < size) sum[threadIdx.x] += inout->Entry[img_offset + idx];
	}

	__syncthreads();

	for (uint_t s = blockDim.x / 2; s >= 1; s >>= 1) {
...
__global__ void cuConcat_v2(uint_t n, 
	const Tensor_t<float4> *input0,
	const Tensor_t<float4> *input1,
	Tensor_t<float4> *output)
{
	uint_t H = input0->nz, W = input0->ny, C0v = input0->nx, C0 = C0v * 4;
	uint_t C1v = input1->nx, C1 = C1v * 4, Cv = C0v + C1v;

	uint_t cv = threadIdx.x + blockIdx.x * blockDim.x, c = cv * 4;
	uint_t w = threadIdx.y + blockIdx.y * blockDim.y;
	uint_t h = (threadIdx.z + blockIdx.z * blockDim.z) % H;
	uint_t img = (threadIdx.z + blockIdx.z * blockDim.z) / H;

	if (cv >= Cv || w >= W || h >= H || img >= Const::num_images) return;

	uint_t gidx = img * H * W * Cv + h * W * Cv + w * Cv + cv;
...
__global__ void cuRelu_v2(uint_t n, 
	...
	const Phase_t *Phase)
{
	...
	uint_t ow = threadIdx.x + blockIdx.x * blockDim.x;
	uint_t oh = threadIdx.y + blockIdx.y * blockDim.y;
	uint_t kv = (threadIdx.z + blockIdx.z * blockDim.z) % Kv, k = kv * 4;
	uint_t img = (threadIdx.z + blockIdx.z * blockDim.z) / Kv;


	if (threadIdx.z + blockIdx.z * blockDim.z >= Kv * Const::num_images) return;
	if (ow >= OW || oh >= OH) return;
	
	uint_t gidx = img * OH * OW * Kv + oh * OW * Kv + ow * Kv + kv;
	uint_t inp_offset = img * H * W * Cv;

	float4 x = Phase->GetBias_v2(kv);

	uint_t rbeg = max((int) 0, (int) oh + PAD - STRIDE * (int) H + 1);
	uint_t rend = min(R, oh + PAD + 1);
	uint_t sbeg = max((int) 0, (int) ow + PAD - STRIDE * (int) W + 1);
	uint_t send = min(S, ow + PAD + 1);

	if ((oh - rbeg + PAD) % STRIDE != 0) ++rbeg;
	if ((ow - sbeg + PAD) % STRIDE != 0) ++sbeg;

	for (uint_t r = rbeg; r < rend; r += STRIDE) {
		uint_t ih = (oh - r + PAD) / STRIDE;
		for (uint_t s = sbeg; s < send; s += STRIDE) {
			uint_t iw = (ow - s + PAD) / STRIDE;
			for (uint_t cv = 0; cv < Cv; ++cv) {
				float4 ii = input->Entry[inp_offset + ih * W * Cv + iw * Cv + cv];
				float4 ff[4];
...
__global__ void cuPostProc_v2(uint_t n, 
	const Tensor_t<float4> *input, 
...
{
	uint_t H = input.nz, W = input.ny, C = input.nx;

	uint_t n = num_images * H * W * C;
	...
	for (uint_t n = 0; n < num_images; ++n) {
		ofstream fout;
		fout.open("img" + to_string(n) + filename);
		for (uint_t h = 0; h < H; ++h) {
			for (uint_t w = 0; w < W; ++w) {
				for (uint_t c = 0; c < C; ++c) {
					uint_t idx = n * H * W * C + h * W * C + w * C + c;
					fout << h << " "
							 << w << " "
							 << c << " " 
							 << buf[idx] << endl;
...
{
	uint_t H = input.nz, W = input.ny, Kv = output.nx;
	
	uint_t OH = H / STRIDE, OW = W / STRIDE;
	
	size_t n = num_images * OH * OW * Kv;
...
void Control::LeakyRelu_v2(const Tensor_t<float4>& input, Tensor_t<float4>& output)
{
	...
	uint_t H = input.nz, W = input.ny, Cv = input.nx;

	dim3 blocks, threads;
	size_t n = num_images * H * W * Cv;
...
{
	uint_t H = inout.nz, W = inout.ny, Cv = inout.nx;
...
{
	uint_t H = input0.nz, W = input0.ny, C0v = input0.nx;
	uint_t C1v = input1.nx;
	uint_t Cv = output.nx;
...
void Control::Conv2DT_v2(const Tensor_t<float4>& input, Tensor_t<float4>& output, const Phase_t& Phase)
{
	...
	uint_t H = input.nz, W = input.ny, Cv = input.nx, K = Phase.conv_dim[2];
	uint_t Kv = output.nx;
