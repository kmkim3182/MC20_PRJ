...
void LayerInfo_t::Predict(map<string, vector<uint>>& shapes)
{
	initial = { 256, 256, 3 };
	...
	uint max_conv_sz = 0, max_convt_sz = 0, max_concat_sz = 0;
...
void Phase_t::Alloc(string names[], map<string, vector<uint>>& shape_map)
{
	num_entries = new uint[num_weights];
	num_shapes = new uint[num_weights];
	pos_entries = new uint[num_weights + 1]; *pos_entries = 0;
	pos_shapes = new uint[num_weights + 1]; *pos_shapes = 0;

	for (uint i = 0; i < num_weights; ++i) {
		string name = names[i];
		vector<uint>& shape = shape_map[name];
		uint sz = 1;
	...
	shapes = new uint[tot_num_shapes];

	for (uint i = 0, p = 0; i < num_weights; ++i) {
		string name = names[i];
		vector<uint>& shape = shape_map[name];
	...
	if (order == 1) {
		bias_index = 0;
		kernel_index = 1;
		memcpy(conv_dim, &shapes[pos_shapes[kernel_index]], 4 * sizeof(uint));
	}
...
void Phase_t::AliasWeights(const Tensor_t<float>& weights, const uint *pos_weights)
{
/*
	uint tot_order;
	...
	uint pos = pos_weights[tot_order - 1];
...
void Phase_t::AliasWeights(const Tensor_t<float4>& weights, const uint *pos_weights)
{
	uint tot_order;
	if (type == ENCODE) tot_order = order + 8;
	else tot_order = order;
...
void Phase_t::AliasShapes(const uint *shapes, const uint *pos_shapes)
{
	uint tot_order;
	if (type == ENCODE) tot_order = order + 8;
	else tot_order = order;

	uint pos = pos_shapes[tot_order - 1];
...
template <typename T>
void Tensor_t<T>::SetSize(const vector<uint>& shape) 
{
	...
	for (vector<uint>::const_iterator i = shape.begin(); i != shape.end(); ++i, ++p) {
		if (p == 0) nx = *i;
		else if (p == 1) ny = *i;
		else if (p == 2) nz = *i;
		else if (p == 3) nw = *i;
...
template <typename T>
void Tensor_t<T>::SetSize(uint x, uint y, uint z, uint w)
{
	nx = x; ny = y; nz = z; nw = w;
	sz = nx * ny * nz * nw;
	dim = 0;
...
template <typename T>
void Tensor_t<T>::Dalloc(const vector<uint>& shape, const T *buf)
{
	if (lAlloc) Destroy();
...
void Control::RegisterWeight(const string& name, vector<uint> shape)
{ 
	shape_map[name] = shape;

	vector<uint> pad = shape;

	for (vector<uint>::iterator i = pad.begin(); i != pad.end(); ++i) {
		*i = (*i / 4 + (*i % 4 == 0 ? 0 : 1)) * 4;
	}
	
	uint dim = (uint) pad.size();
	...
	for (vector<uint>::reverse_iterator i = shape.rbegin(), j = pad.rbegin(); i != shape.rend(); ++i, ++j, --s) {
		S[s] = *i;
		P[s] = *j;
	}
	...
	for (uint a = 0; a < S[0]; ++a) {
		for (uint b = 0; b < S[1]; ++b) {
			for (uint c = 0; c < S[2]; ++c) {
				for (uint d = 0; d < S[3]; ++d, ++p) {
...
{
	uint num_funcs = WeightInfo.num_funcs;
	
	WeightInfo.nw = NUM_WEIGHTS;

	WeightInfo.pos = new uint[num_funcs + 1]; *WeightInfo.pos = 0;
	WeightInfo.pos_padded = new uint[num_funcs + 1]; *WeightInfo.pos_padded = 0;
	...
	for (uint i = 1; i <= 8; ++i) {
		...
		for (uint j = 0; j < num_weights; ++j, ++p) {
			vector<uint>& pad = pad_map[weights[j]];
			vector<uint>& shape = shape_map[weights[j]];
			
			uint psz = 1, sz = 1;
			
			for (vector<uint>::iterator k = pad.begin(), l = shape.begin(); k != pad.end(); ++k, ++l) {
				psz *= (*k);
				sz *= (*l);
			}
...
void Control::SortPhase()
{
	...
	*pos_shapes = 0;
	...
	p = 0;
	for (uint i = 1; i <= 8; ++i, ++p)
		memcpy(&shapes[pos_shapes[p]], Decoder[i].shapes, Decoder[i].tot_num_shapes * sizeof(uint));
	for (uint i = 1; i <= 8; ++i, ++p)
		memcpy(&shapes[pos_shapes[p]], Encoder[i].shapes, Encoder[i].tot_num_shapes * sizeof(uint));
}
...
void Control::Initialize()
{
	...
	WeightInfo.position = new uint[NUM_WEIGHTS];
...
{
	int num_images = (int) _num_image;

	sendcounts = new int[MPI_GLOBAL_SIZE];
	displs = new int[MPI_GLOBAL_SIZE + 1]; *displs = 0;

#ifdef USE_MPI
	MPI_Bcast(&num_images, 1, MPI_INT, 0, MPI_COMM);
#endif

	for (int i = 0; i < MPI_GLOBAL_SIZE; ++i) {
		sendcounts[i] = num_images / MPI_GLOBAL_SIZE;
		if (i < num_images % MPI_GLOBAL_SIZE) ++sendcounts[i];
	}
	
	num_images = sendcounts[MPI_GLOBAL_RANK];
	
	for (int i = 0; i < MPI_GLOBAL_SIZE; ++i) {
		sendcounts[i] *= (int) num_pixels;
		displs[i + 1] = displs[i] + sendcounts[i];
	}
	
	this->num_images = (uint) num_images;
}
...
void Control::ScatterImages(uint8_t *&_input_buf, float *&_weights)
{
	...
	if (MPI_GLOBAL_RANK != 0) {
		_input_buf = new uint8_t[num_data];
	...
	if (MPI_GLOBAL_RANK != 0) {
		MPI_Irecv(_input_buf, num_data, MPI_UINT8_T, 0,
			MPI_GLOBAL_RANK, MPI_COMM, &requests[num_requests++]);
	}
	else {
		for (int i = 1; i < MPI_GLOBAL_SIZE; ++i) {
			MPI_Isend(&_input_buf[displs[i]], sendcounts[i], MPI_UINT8_T, i,
				i, MPI_COMM, &requests[num_requests++]);
		}
	}
	...
	cudaCheckError(
		cudaMemcpy(input_buf.Entry, _input_buf, num_data * sizeof(uint8_t), cudaMemcpyHostToDevice)
	);
...
void Control::Alloc(uint8_t *_input_buf, float *_weights, size_t _num_image)
{
	...
	cudaCheckError( cudaMemcpyToSymbol(&Const::num_images, &num_images, sizeof(uint)) );
	...
	for (int i = 1; i <= 8; ++i) {
		vector<uint>& convt = Layer.convt[i];
		if (i != convt_max) {
	...
	for (int i = 1; i <= 8; ++i) {
		vector<uint>& concat = Layer.concat[i];
		if (i == 1) {
...
#ifdef USE_MPI
	if (MPI_GLOBAL_RANK != 0) _output_buf = new uint8_t[num_data];
#endif

	cudaCheckError( 
		cudaMemcpy(_output_buf, output_buf.Entry, num_data * sizeof(uint8_t), cudaMemcpyDeviceToHost) 
	);

#ifdef USE_MPI
	if (MPI_GLOBAL_RANK != 0) {
		MPI_Isend(_output_buf, num_data, MPI_UINT8_T, 0, 
			MPI_GLOBAL_RANK, MPI_COMM, &requests[num_requests++]);
	}
	else {
		for (int i = 1; i < MPI_GLOBAL_SIZE; ++i) {
			MPI_Irecv(&_output_buf[displs[i]], sendcounts[i], MPI_UINT8_T, i,
				i, MPI_COMM, &requests[num_requests++]);
		}
	}
...
void Control::PrintTimerInfo()
{
	...
	if (MPI_GLOBAL_RANK == 0) {
		const uint num_timers = NUM_OPERATIONS;
		cout << "\n";	
